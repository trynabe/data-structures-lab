...
src_bwi = G4.get_vertex('BWI')
result_bwi = shortest_path_lengths(G4, src_bwi)
print("Source: BWI")
for v, dist in result_bwi.items():
  print(f"{v}: {dist}")

print("\n--- All other sources ---")
for v in G4.vertices():
  if str(v) != 'BWI':
    res = shortest_path_lengths(G4, v)
    print(f"\nSource: {v}")
    for dest, dist in res.items():
      print(f"  {dest}: {dist}")